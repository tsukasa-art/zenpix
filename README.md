...
native 1.0.4にはこのSIMD経路が含まれます。GitHub Actionsの5 native環境ではSIMD版と強制scalar版を同じsourceからbuildし、C test、共有ライブラリFFI比較、AVIF roundtrip、runtime依存検査を実行します。各jobは直前にbuildしたbinaryをpackし、SHA256一致とNode.js / Bun / Deno API、CLI実変換を確認します。集約jobはroot、5 native optional packages、WASMの計7 tarballについてversion・必須ファイル・licenseを検査します。CI上の配布検査と公開後のregistry再取得・実機利用は別の証拠として扱います。
...
表の「対象」はpackageとCI workflowの対象を示します。1.0.4はmacOS 12以上、glibc 2.34以上のLinux、Windows x64を対象とし、Linux CIは`GLIBC_2.34`より新しいsymbol参照を拒否します。Alpine Linux（musl）とWindows arm64のビルド済みpackageは提供していません。

macOS版1.0.4はcodecを静的リンクし、macOS 12.0をdeployment targetとしてbuild・検査します。CIは外部codec dylib依存がないこと、`minos 12.0`、packed API / CLI実変換を確認します。
