 ...
 * 各 fixture をアスペクト比を保持したままリサイズし、zenpix と sharp で AVIF 出力して比較用サンプルを生成する。
 * fit: "inside" でクロップなし・比率保持。エンコード差のみを比較できる。
 *
 * Run:
...
const OUT_W = 960;
const OUT_H = 960; // fit: "inside" なので長辺が収まる最大値。実際の高さは画像比率で決まる。
...
for (const fx of FIXTURES) {
  ...
  // 共通のリサイズ済み PNG（アスペクト比保持・クロップなし）
  const resizedPng = await sharp(src)
    .resize(OUT_W, OUT_H, { fit: "inside" })
    .png()
    .toBuffer();

  // ── sharp ───────────────────────────────────────────────────────────────
  const sharpBuf = await sharp(resizedPng)
  ...
  // ── zenpix ──────────────────────────────────────────────────────────────
  const img  = decode(resizedPng);
  const avif = encodeAvif(img, { quality: QUALITY, speed: SPEED });
