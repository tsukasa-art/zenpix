...
native 1.0.4はRGBA用NEON / SSE2を5環境でbuild・testし、各CI jobが直前にbuildしたbinaryをpackしてSHA256一致、runtime依存、Node.js / Bun / Deno API、CLI実変換を検査します。集約jobはroot、5 native optional packages、WASMの計7 tarballをversion・必須ファイル・license・SHA256で検査します。CI上の配布検査と公開後のregistry再取得・実機利用は別の証拠として扱います。
