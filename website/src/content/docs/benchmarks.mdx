...
Reproducing those three outputs with the same 4093×2894 fixture took about 7.6 seconds and peaked at roughly 0.5–0.6 GiB in a constrained Linux container on an M4 Pro; the full-resolution AVIF accounted for most of the time. The original system logs are unavailable, so we cannot distinguish an upstream timeout from memory pressure conclusively. This supports the narrower conclusion that the request design exceeded the VPS latency budget, not a general claim about Sharp's CPU or memory behavior.
...
This virtualized run did not reproduce either “Sharp consumes both cores” or “zenpix uses less memory.” It is not generalized to a real VPS, concurrent requests, or other fixtures. Full conditions and numbers are recorded in `docs/reference/benchmarks.md` on GitHub.
