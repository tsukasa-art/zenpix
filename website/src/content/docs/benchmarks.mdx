...
## Unpublished source: scalar vs SIMD

On 2026-07-31, the unpublished RGBA NEON path was compared with a forced-scalar build from the same source on an Apple M4 Pro running macOS arm64. Each trial used three warm-ups and 15 alternating scalar/SIMD pairs; the complete measurement was repeated three times.

| Workload | Threads | Range of median speedups across three trials |
|---|---:|---:|
...
The input fixture is not published in a redistributable form. RGBA improved in this environment; RGB is outside the SIMD path and did not improve. x86_64 SSE2 correctness was checked under Rosetta, but native Intel performance remains unverified. These results are not evidence that every zenpix workload is faster.
